...
for (x in gap_fee_range) {
  for (y in privately_billed_range) {
    ...
    profit_loss <- add_row(
      profit_loss,
      gap = x, private_billed = y, profit = profit, profit_rel = profit/current_fee_mean
    )
  }
}
...
fig_plotly_3d <- plot_ly(
  data = profit_loss,
  x = ~gap, y = ~private_billed, z = ~profit, text = ~profit_rel * 100,
  type = "scatter3d", mode = "markers",
  hovertemplate = paste(
    "Gap fee: <b>%{x}</b><br>",
    "Private: <b>%{y}</b><br>",
...
fig_ggplotly <- ggplot(
  ...
  aes(
    ...
    text = sprintf(
      "Gap: $%d<br>Private-billed: %d%%<br>Profit: $%.2f<br>Profit (%%): %s%%",
      gap, private_billed,profit, profit_rel
    )
  )) +
  geom_point(
    aes(fill = profit),
