...
for (x in gap_fee_range) {
  for (y in concessional_bulkbilled_range) {
    ...
    benefit_loss <- add_row(benefit_loss, gap = x, concessional_bulkbilled = y, benefit = benefit)
  }
}
...
fig_plotly_3d <- plot_ly(
  data = benefit_loss, x = ~gap, y = ~concessional_bulkbilled, z = ~benefit,
...
\small
\begin{aligned}
\text{where }b &= \text{proportion of patients bulk-billed concessionally (0-1)}\\\\\\
