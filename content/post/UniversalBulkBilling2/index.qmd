...
*Dashboard available on [Amazon Web Services](https://shiny.davidfong.org/universalbulkbilling/) and [ShinyApps](https://davidfong.shinyapps.io/universalbulkbilling).*
...
-   Broadening the current *bulk-billing incentive* for individual consultations/services to all patients.
    - The government had provided *bulk-billing incentives* for general practitioners to bulk-bill patients previously deemed to benefit more from bulk-billing. The value of these incentives have been 7.35 to 21.85 dollars per service in urban areas.
...
-   If the general practice agrees to bulk-bill all patients, i.e. universal bulk-billing, the government will pay the practice an additional 12.5% over and above the government-set price for each fee-for-service item. This incentive payment is sometimes called a 'loading payment'. Half (50%) of this payment is to be paid to the practice, [the other half (50%) is to be paid to the individual doctor](https://www.medicalrepublic.com.au/practices-gps-to-split-new-pip-50-50/119243).
    - On 25th March 2025, the service items which needed to be bulk-billed, and attract the 'loading' payment were defined in the document [Bulk Blling Practice Incentive Program: Eligible Services](https://www.health.gov.au/resources/publications/bulk-billing-practice-incentive-program-eligible-services)
    - The definition of which service items need to be 'bulk-billed', and which service items will attract the 12.5% loading was not initially fully defined in the government-provided [fact sheet](https://mcusercontent.com/8a3d58d1cfb663c4dcefbc00d/files/bd321db2-d98f-0c87-efb9-1456a6053389/More_bulk_billing_fact_sheet.pdf) and [RACGP FAQ](https://www.racgp.org.au/advocacy/advocacy-resources/medicare-announcement-member-faqs), although they did suggest that consultation items A/B/C/D, mental health care plan and other GP non-referred attendances will attract the 12.5% loading.
...
For more discussion about the mathematical relationship between benefit-per-service, relative benefit and mean fees, mean gap-fees and proportion of existing services which currently receive individual bulk-billing incentives, see `['Further Mathematical exploration']({{< ref "#further-mathematical-exploration" >}})`{=markdown}
...
# fees correct as of 28 Sep 2025
fee_names <- c("a", "b", "c", "d", "e")
service_fees <- array(
  data = c(20.05, 43.90, 84.90, 125.10, 202.65),
...
If you 'Run Code' the block below With the default settings of 50% of patients are concessionally bulk-billed and a mean gap fee of 50, the benefit-per-patient is shown as `-7.84`, i.e. a loss of $7.84 compared to the current mean fee of `87.76`.
...
paste("Total Benefit-loss: ", formatC(benefit_loss, digits = 2, format = "f"))
paste("Quarterly UBB incentive:", formatC(ubb_incentive, digits = 2, format = "f"))
```
...
paste("Current mean fee: ", formatC(current_fee_mean, digits = 2, format = "f"))
paste("Total Benefit-loss: ", formatC(benefit_loss, digits = 2, format = "f"))
paste("UBB quarterly incentive:", formatC(ubb_incentive, digits = 2, format = "f"))
```

If the mean gap fee is set to 35, the average consult length is longer and 80% of patients are concessionally bulk-billed, the practice can expect a mean benefit of `5.66` per service more than the current mean fee of `91.91`. Note that this practice has a higher current mean fee than the first practice, but longer consultation times would likely result in a lower fee-rate per hour.
...
Compared to the `[second practice model]({{< ref "#modifying-the-model-for-a-different-practice" >}})`{=markdown} described above, adding care plan item numbers increases the mean benefit from `5.66` to `6.95` per service. The current mean fee has also risen, from `91.91` to `97.81`, reflecting the higher value of service items included.
...
# old gpmp/tca/cdmrv item numbers, superseded in second half of 2025
# new_fee_names <- c(
#   fee_names,
...
new_fee_names <- c(
  fee_names,
  # append items 965, 967
  # GP chronic condition management plan and review
  c("GPCCMP", "GPCCMP RV")
)
new_service_fees <- array(
  # append fees for 965 and 967
  data = c(service_fees, 156.55, 156.55),
  dim = c(length(new_fee_names), 1),
  dimnames = list(new_fee_names, "fee")
)
...
new_gap_fee <- c(
  rep(35, 5),
  # no gap fees charged for GPCCMP and review,
  # even 'private' patients
  rep(0, 2)
)
new_individual_bulkbill_incentive_by_fee <- c(
  individual_bulkbill_incentive_by_fee,
  # only 'single' incentive for GPCCMP and review
  rep("single", 2)
)
# Set the privately-billed proportion to 0.2 (20%), the mean gap fee to $35
# and new service proportions
new_practice_service_raw <- 
  ...
  # and GPCCMP and reviews
  # based on combination of July 2024-June 2025 (for ABCDE) and
  # July 2025 statistics (for GPCCPMP and GPCCMP RV)
  # as no GPCCMP (965) had been charged prior to July 2025,
  # no GPCCMP review (967) could be charged in July 2025
  # (as a review '967' could only be charged 3+ months after a '965')
  c(950, 3956, 4171, 1146, 75, 941, 0)
new_practice_service_proportions <- 
  new_practice_service_raw/sum(new_practice_service_raw)
...
# Show the value of average net_benefit
paste("Current mean fee: ", formatC(current_fee_mean, digits = 2, format = "f"))
paste("Benefit-loss: ", formatC(benefit_loss, digits = 2, format = "f"))
paste("UBB quarterly incentive:", formatC(ubb_incentive, digits = 2, format = "f"))
```
...
ggplot(
  data = gap_vs_benefit, 
  mapping = aes(x = gap, y = benefit_loss)
  ) +
  geom_point() +
  labs(
    x = "Mean Gap fee ($)",
...
new_fee_names <- c(
  fee_names,
  # append items 'care plan' (965) and 'care plan review' (967)
  c("GPCCMP", "GPCCMP RV")
)
new_service_fees <- array(
  # append fees for 965 and 967
...
concessional_bulkbilled_vs_benefit <- tibble(
  concessional_bulkbilled = concessional_bulkbilled_range,
  benefit_loss = benefit_loss
)

ggplot(
  data = concessional_bulkbilled_vs_benefit, 
  mapping = aes(x = concessional_bulkbilled, y = benefit_loss)
  ) +
  geom_point() +
  labs(
...
new_fee_names <- c(
  fee_names,
  # append chronic condition management plan item numbers (965, 967)
...
new_service_fees <- array(
  # append fees for 965, 967 and health assessments and mental health plans
  data = c(service_fees,
           156.55, 156.55,
           69.2, 160.85, 222, 313.6, 247.65,
           83.65, 123.15, 106.2, 156.45, 83.65
           ),
  dim = c(length(new_fee_names), 1),
  dimnames = list(new_fee_names, "fee")
)
...
new_practice_service_raw <- 
  ...
  # and care plans (CCMP), health assessment and mental health plan items
  c(4037130, 81240293, 19899953, 1609147, 174664,
    # the following two numbers are for care plans (965) and care plan review (967)
    # they are 'imputed' from July 2025 statistics (965/967 replaced 721/723/732 in July 2025)
    # keeping them in the same proportion to ABCDE (2024) as they were in July 2025
    6982325, 260080,
...
for (x in gap_fee_range) {
  for (y in concessional_bulkbilled_range) {
    ...
    benefit_loss <- add_row(
      ...
      benefit = benefit, ubb_incentive = ubb_incentive
    )
  }
}
...
fig_plotly_3d <- plot_ly(
  data = benefit_loss, 
  x = ~gap, y = ~concessional_bulkbilled, z = ~benefit, customdata = ~ubb_incentive,
  type = "scatter3d", mode = "markers",
  hovertemplate = paste(
    "Mean Gap fee: <b>$%{x}</b><br>",
    "Private: <b>%{y}</b>%<br>",
    "Benefit: <b>%{z:$.2f}</b><br>",
    "UBB incentive: <b>%{customdata:$.2f}</b>",
    "<extra></extra>"
  ),
  marker = list(
    color = ~benefit,
...
fig_ggplot <- ggplot(
  ...
  aes(x = gap, y = concessional_bulkbilled, fill = benefit)) +
  theme_bw() + geom_tile() + 
...
# you will need to copy this code (and all the pre-requisites)
# into a 'R/RStudio' and run it yourself :)
benefit_loss_practice_standard <- benefit_loss_practice_standard |>
...
fig_plotly_3d <- plot_ly(
  ...
  text = ~benefit_rel * 100, customdata = ~custom_data_list,
  type = "scatter3d", mode = "markers",
  hovertemplate = paste(
    "Mean Gap fee: <b>$%{x}</b><br>",
...
new_practice_service_raw_2 <- 
  # A, B, C, D, E service items
  # GPCCMP, health assessment and mental health plan items
  c(950, 3956, 4171, 1146, 75,
    941, 0,
    0, 13, 101, 45, 1,
    7, 5, 64, 31, 41)
...
fig_ggplot <- ggplot(
  ...
  ggtitle("Net benefit to practice, per service") +
  scale_fill_gradientn(colors = rainbow(5), limits = c(-45, 45))
# to view thee above plot (the 'conventional table' below), use
# 'fig_ggplot + theme_tufte()'
...
new_practice_service_raw <- 
  # A, B, C, D, E service items
  # and GPCCMP care plan, health assessment and mental health plan items
  c(950, 3956, 4171, 1146, 75,
    941, 0,
    0, 13, 101, 45, 1,
    7, 5, 64, 31, 41)
...
fig_ggplot <- ggplot(
  data = benefit_loss |> 
    dplyr::filter(gap == (gap %/% 5 * 5), concessional_bulkbilled == (concessional_bulkbilled %/% 5 * 5)),
  aes(x = gap, y = concessional_bulkbilled, fill = benefit_rel)) +
  theme_bw() + geom_tile() + 
  geom_text(aes(label=round(benefit_rel, 0)), size = 8/.pt) +
  labs(
    x = "Mean Gap fee ($)",
    y = "Concessional bulk-billed (%)",
    fill = "Revenue change (%)"
...
# the table shown in the government-provided fact sheet used 'old' MBS rebates
old_fee_names <- c("a", "b", "c", "d", "e")
old_service_fees <- array(
  data = c(19.6, 42.85, 82.9, 122.15, 197.9),
  dim = c(length(old_fee_names), 1),
  dimnames = list(old_fee_names, "fee")
)
# just include service items B, C and D
short_service_list <- old_service_fees[c("b", "c", "d"),]

old_individual_bulkbill_incentive <- array(
  # there are two different individual bulk-billing incentive fees,
  # 'standard' and 'triple'
  data = c(
    # standard bulk-billing incentive applies to service items like item A/'3'
  ...
  tibble(
    ...
    Metro = num(
      short_service_list*(1+universal_bulkbill_incentive)+
        old_individual_bulkbill_incentive[1, 2],
      digits = 2
    ),
    Regional = num(
      short_service_list*(1+universal_bulkbill_incentive)+
        old_individual_bulkbill_incentive[2, 2],
      digits = 2
    ),
    `Large Rural` = num(
      short_service_list*(1+universal_bulkbill_incentive)+
        old_individual_bulkbill_incentive[3, 2],
      digits = 2
    ),
    `Small Rural` = num(
      short_service_list*(1+universal_bulkbill_incentive)+
        old_individual_bulkbill_incentive[4, 2],
      digits = 2
    ),
    Remote = num(
      short_service_list*(1+universal_bulkbill_incentive)+
        old_individual_bulkbill_incentive[5, 2],
      digits = 2
    ),
  )
...
\small
\begin{aligned}
\text{where } u &= \text{rebates (100%) plus quarterly loading payment (12.5%)} = 1.125\\\\\\
x &= \text{ mean service fee}\\\\\\
i &= \text{ mean individual service bulk-billing incentive}\\\\\\
\end{aligned}
$$
...
*This article is an updated version of [Calculating the practice benefit of universal bulk-billing](../UniversalBulkBilling). Updated model includes MBS Service Level A and, optionally, care plans (721/723/732).*

*Update: includes display of Universal-Bulk-Bill incentive payment, paid quarterly with PIP (13 Sep 2025)*

*Update: includes new GPCCMP item numbers (965/967), replacing GPMP/TCA item numbers (721/723)*